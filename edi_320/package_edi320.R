...
# This script converts the parent data package knb-lter-bnz.530.x into the
...
path <- '/Users/csmith/Documents/EDI/datasets/ecocomDP/edi_320'
parent_pkg_id <- 'knb-lter-bnz.530.9'
child_pkg_id <- 'edi.320.1'

# Convert knb-lter-bes.543.x to ecocomDP

message('Creating ecocomDP tables')

convert_bnz530_to_ecocomDP(path, parent_pkg_id, child_pkg_id)
...
use_i <- catvars$code == 'X..ha'
catvars$definition[use_i] <- 'Number of individuals per hectare'
catvars$unit <- 'dimensionless'
...
ecocomDP::make_eml(
  data.path = path,
  code.path = path,
  code.files = c('convert_bnz530_to_ecocomDP.R', 'package_edi320.R'),
