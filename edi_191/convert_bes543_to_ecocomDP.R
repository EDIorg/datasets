...
convert_bes543_to_ecocomDP <- function(path, parent_pkg_id, child_pkg_id){
  ...
  taxon <- data.frame(
    ...
    taxon_rank = rep(NA_character_, nrow(taxa_resolved)),
    taxon_name = rep(NA_character_, nrow(taxa_resolved)),
    authority_system = rep(NA_character_, nrow(taxa_resolved)),
    authority_taxon_id = rep(NA_character_, nrow(taxa_resolved)),
    stringsAsFactors = F
  )
  ...
  taxon$taxon_name <- taxa$common_name
  ...
  taxon$taxon_id <- taxa$species_id[
    match(taxon$taxon_name, taxa$common_name)
  ]
