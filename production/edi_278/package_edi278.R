...
path <- 'C:/Users/Colin/Documents/EDI/data_sets/ecocomDP/edi_278'
parent_pkg_id <- 'knb-lter-sbc.119.2'
child_pkg_id <- 'edi.278.1'

# Convert knb-lter-bes.543.x to ecocomDP

message('Creating ecocomDP tables')

convert_sbc50_to_ecocomDP(path, parent_pkg_id, child_pkg_id)
...
ecocomDP::make_eml(
  data.path = path,
  code.path = path,
  code.files = c('convert_sbc119_to_ecocomDP.R', 'package_edi278.R'),
