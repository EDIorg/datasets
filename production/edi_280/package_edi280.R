...
# This script converts the parent data package knb-lter-arc.10317.x into the
...
path <- 'C:/Users/Colin/Documents/EDI/data_sets/ecocomDP/edi_280'
parent_pkg_id <- 'knb-lter-arc.10317.7'
child_pkg_id <- 'edi.280.1'

# Convert knb-lter-bes.543.x to ecocomDP

message('Creating ecocomDP tables')

convert_arc10317_to_ecocomDP(path, parent_pkg_id, child_pkg_id)
...
catvars$definition <- 'average percent cover'
catvars$unit <- 'percent'
...
ecocomDP::make_eml(
  data.path = path,
  code.path = path,
  code.files = c('convert_arc10317_to_ecocomDP.R', 'package_edi280.R'),
