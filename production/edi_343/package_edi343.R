# This script converts the parent data package knb-lter-knz.88 into the
...
path <- 'C:/Users/Colin/Documents/EDI/data_sets/ecocomDP/edi_343'
parent_pkg_id <- 'knb-lter-knz.88.7'
child_pkg_id <- 'edi.343.1'

# Convert to ecocomDP

message('Creating ecocomDP tables')

convert_knz88_to_ecocomDP(path, parent_pkg_id, child_pkg_id)
...
catvars$definition[use_i] <- 'Number of taxa observed'
...
ecocomDP::make_eml(
  data.path = path,
  code.path = path,
  code.files = c('convert_knz88_to_ecocomDP.R', 'package_edi343.R'),
