# This script converts the parent data package knb-lter-knz.130 into the
...
path <- 'C:/Users/Colin/Documents/EDI/data_sets/ecocomDP/edi_344'
parent_pkg_id <- 'knb-lter-knz.130.4'
child_pkg_id <- 'edi.344.1'

# Convert to ecocomDP

message('Creating ecocomDP tables')

convert_knz130_to_ecocomDP(path, parent_pkg_id, child_pkg_id)
...
use_i <- catvars$code == 'ScienceName'
catvars$definition[use_i] <- 'Scientific name'
...
ecocomDP::make_eml(
  data.path = path,
  code.path = path,
  code.files = c('convert_knz130_to_ecocomDP.R', 'package_edi344.R'),
