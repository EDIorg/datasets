# This script converts the parent data package knb-lter-hbr.178 into the
...
path <- 'C:/Users/Colin/Documents/EDI/data_sets/ecocomDP/edi_348'
parent_pkg_id <- 'knb-lter-hbr.178.2'
child_pkg_id <- 'edi.348.1'

# Convert to ecocomDP

message('Creating ecocomDP tables')

convert_hbr178_to_ecocomDP(path, parent_pkg_id, child_pkg_id)
...
ecocomDP::make_eml(
  data.path = path,
  code.path = path,
  code.files = c('convert_hbr178_to_ecocomDP.R', 'package_edi348.R'),
