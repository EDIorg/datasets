# This script converts the parent data package knb-lter-sev.23 into the
...
path <- 'C:/Users/Colin/Documents/EDI/data_sets/ecocomDP/edi_334'
parent_pkg_id <- 'knb-lter-sev.23.121705'
child_pkg_id <- 'edi.334.1'

# Convert to ecocomDP

message('Creating ecocomDP tables')

convert_sev23_to_ecocomDP(path, parent_pkg_id, child_pkg_id)
...
ecocomDP::make_eml(
  data.path = path,
  code.path = path,
  code.files = c('convert_sev23_to_ecocomDP.R', 'package_edi334.R'),
