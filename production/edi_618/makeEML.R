...
file_names <- c('bbwm_deposition_all.csv',
                'bbwm_deposition_annual.csv',
                'bbwm_deposition_monthly.csv')

table_names <- c('precipitation chemistry for all samples collected',
                 'annual ambient atmospheric deposition fluxes',
                 'monthly ambient atmospheric deposition fluxes')

dataset_title <- 'The Bear Brook Watershed in Maine, USA: Long-term atmospheric deposition chemistry 1987 - 2012'

file_descriptions <- c('precipitation chemistry for all samples collected',
                       'annual ambient atmospheric deposition fluxes',
                       'monthly ambient atmospheric deposition fluxes')

quote_character <- rep("\"",3)

temp_cov <- c('1987', '2012')
