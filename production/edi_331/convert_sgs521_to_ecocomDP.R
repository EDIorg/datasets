...
#
# This script converts knb-lter-sgs.521 to the ecocomDP.

convert_sgs521_to_ecocomDP <- function(path, parent_pkg_id, child_pkg_id){
  
  message(paste0('Converting ', parent_pkg_id, ' to ecocomDP (', child_pkg_id, ')' ))
  
  project_name <- 'vegitation_stress_basal_cover'
  ...
  data$event_key <- paste0(
    data$Year,
    data$location_id
  )
  
  map_event_id <- data.frame(
    key = unique(data$event_key),
    id = paste0(
      'ev_',
      seq(length(unique(data$event_key)))
    )
  )
  
  data$event_id <- map_event_id$id[
    match(
      data$event_key,
      map_event_id$key
    )
