# This script converts the parent data package knb-lter-knz.29 into the
...
path <- 'C:/Users/Colin/Documents/EDI/data_sets/ecocomDP/edi_341'
parent_pkg_id <- 'knb-lter-knz.29.12'
child_pkg_id <- 'edi.341.1'

# Convert to ecocomDP

message('Creating ecocomDP tables')

convert_knz29_to_ecocomDP(path, parent_pkg_id, child_pkg_id)
...
use_i <- catvars$code == 'Comments_about_individuals'
catvars$definition[use_i] <- 'Comments'

use_i <- catvars$code == 'Comments_about_instars'
catvars$definition[use_i] <- 'Comments'
...
ecocomDP::make_eml(
  data.path = path,
  code.path = path,
  code.files = c('convert_knz29_to_ecocomDP.R', 'package_edi341.R'),
