# This script converts the parent data package knb-lter-pie.404 into the
...
path <- 'C:/Users/Colin/Documents/EDI/data_sets/ecocomDP/edi_338'
parent_pkg_id <- 'knb-lter-pie.404.1'
child_pkg_id <- 'edi.338.1'

# Convert to ecocomDP

message('Creating ecocomDP tables')

convert_pie404_to_ecocomDP(path, parent_pkg_id, child_pkg_id)
...
use_i <- catvars$code == 'Fraction of total chlorophyll a'
catvars$definition[use_i] <- 'Chlorophyll a concentration associated with taxa group'
catvars$unit[use_i] <- 'microgramPerLiter'
...
ecocomDP::make_eml(
  data.path = path,
  code.path = path,
  code.files = c('convert_pie404_to_ecocomDP.R', 'package_edi338.R'),
