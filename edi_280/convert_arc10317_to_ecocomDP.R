...
#
# This script converts knb-lter-arc.10317.x to the ecocomDP.

convert_arc10317_to_ecocomDP <- function(path, parent_pkg_id, child_pkg_id){
  
  message(paste0('Converting ', parent_pkg_id, ' to ecocomDP (', child_pkg_id, ')' ))
  
  project_name <- 'Kuparuk_Moss'
  ...
  data <- read.csv(
    file = entity_info$urls[stringr::str_detect(entity_info$names, 'Moss_csv')], 
    header = T, 
    as.is = T
  )
  ...
  data$Habitat..sub.site.[data$Habitat..sub.site. == 'N/A'] <- NA_character_
  ...
  taxon_id_map <- data.frame(
    key = unique(data$Species.Cover.type),
    id = paste0('tx_', seq(length(unique(data$Species.Cover.type)))),
    stringsAsFactors = F
  )
  
  data$taxon_id <- taxon_id_map$id[
    match(data$Species.Cover.type, taxon_id_map$key)
  ]
  ...
  names(observation)[
    names(observation) == 'Mean....AVERAGE.'
    ] <- 'value'
  
  observation$variable_name <- 'Mean....AVERAGE.'
  ...
  location <- ecocomDP::make_location(
    x = data,
    cols = c('River', 'Station..km.', 'Reach', 'Habitat..sub.site.', 'location_id')
  )
  ...
  use_i <- variable_mapping$variable_name == 'Mean....AVERAGE.'
