# This script converts the parent data package knb-lter-sgs.520 into the
...
path <- 'C:/Users/Colin/Documents/EDI/data_sets/ecocomDP/edi_333'
parent_pkg_id <- 'knb-lter-sev.29.175390'
child_pkg_id <- 'edi.333.1'

# Convert to ecocomDP

message('Creating ecocomDP tables')

convert_sev29_to_ecocomDP(path, parent_pkg_id, child_pkg_id)
...
use_i <- catvars$code == 'NUMBER_TOTAL_PER_PLOT'
catvars$definition[use_i] <- 'Number of species per 0.25 meter squared'
catvars$unit[use_i] <- 'numberPerMeterSquared'
...
ecocomDP::make_eml(
  data.path = path,
  code.path = path,
  code.files = c('convert_sgs520_to_ecocomDP.R', 'package_edi330.R'),
