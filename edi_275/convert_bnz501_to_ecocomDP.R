...
#
# This script converts knb-lter-bnz.501.x to the ecocomDP, validates the 
# resultant tables, and creates the associated EML record.

path <- '/Users/csmith/Downloads/edi_275'
parent_pkg_id <- 'knb-lter-bnz.501.17'
child_pkg_id <- 'edi.275.1'

package_edi_275 <- function(path, parent_pkg_id, child_pkg_id){
  ...
  ecocomDP::make_eml(
    data.path = path,
    code.path = path,
    code.files = 'convert_bnz501_to_ecocomDP.R',
...
# Create ecocomDP child from the parent data package
