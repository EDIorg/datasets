...
path <- 'C:/Users/Colin/Documents/EDI/data_sets/ecocomDP/edi_279'
parent_pkg_id <- 'knb-lter-mcr.8.30'
child_pkg_id <- 'edi.279.1'

# Convert knb-lter-bes.543.x to ecocomDP

message('Creating ecocomDP tables')

convert_mcr8_to_ecocomDP(path, parent_pkg_id, child_pkg_id)
...
ecocomDP::make_eml(
  data.path = path,
  code.path = path,
  code.files = c('convert_mcr8_to_ecocomDP.R', 'package_edi279.R'),
