...
table_names <- trimws(c("LAGOSNE_lakeslocus101.csv"))

# Data table descriptions (order must follow the above listing of tables).

data_table_descriptions <-trimws(c("Lake location and physical characteristics."))
...
data_table_urls <- trimws(c("https://lter.limnology.wisc.edu/sites/default/files/data/LAGOS_NE_LOCUS_EXPORT/LAGOSNE_lakeslocus101.csv"))
...
dataset_title <- trimws("LAGOS-NE-LOCUS v1.01: A module for LAGOS-NE, a multi-scaled geospatial and temporal database of lake ecological context and water quality for thousands of U.S. Lakes: 1925-2013")
...
data_package_id <- trimws("edi.62.1")
