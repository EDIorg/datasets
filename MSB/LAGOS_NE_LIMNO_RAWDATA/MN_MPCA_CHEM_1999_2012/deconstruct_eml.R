      ...
      if (!is.null(ref_values_out[[i]])){
        ...
        for (j in 1:length(element_names)){
          ...
          if (attr(match_info, "match.length") > 0){
            ...
            if (length(base_elements[[j]][[2]]) > 1){
              
              base_elements[[j]][[2]] <- as(base_elements[[j]][[2]], element_slot_pairs[[j]])
