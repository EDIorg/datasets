...
path <- "C:\\Users\\Colin\\Documents\\EDI\\data_sets\\msb_csi_lagos\\CT_POST_NUTRIENTS"
dataset.name <- "CT_POST_NUTRIENTS"
table.name <- "CT_POST_NUTRIENTS.csv"
xml.file.name <- "CT_POST_NUTRIENTS.xml"


data.package.id <- "edi.13.1"
...
xml_file_names <- trimws(c("CT_DEEP_CHEM_GRANT_PROGRAM.xml"))
...
elements <- deconstruct_eml(path = path, xml.file.name = xml_file_names[1])
...
dataset@creator <- elements[["creator"]]
...
for (i in 1:length(elements[["associated_party"]])){
  ...
  if (!is.na(elements[["associated_party"]][i])){
    ...
    if (length(reference) == 0){
      ...
      list_of_associated_party[[i]] <- associated_party[[i]]
...
for (i in 1:length(table_names)){
  ...
  elements <- deconstruct_eml(path = path, xml.file.name = xml_file_names[i])
...
if (nrow(custom_units_df) == 1 & sum(is.na(custom_units_df)) > 0){
  custom_units <- "no"
} else {
  custom_units <- "yes"
}
