...
write_reconstructed_attributes <- function(path, table.name, dataset.name, xml.file.name){
  ...
  if (answer == "y"){
    ...
    # List column names not present in the attributeList
    ...
    attributes_not_as_columns[k+1] <- paste("Attribute length = ", as.character(length(xml_attribute_names)),
                                            "Column numbers = ", as.character(length(table_column_names)))
    
    print("Data table attributes not listed in the EML. NA = no discrepancy.")
