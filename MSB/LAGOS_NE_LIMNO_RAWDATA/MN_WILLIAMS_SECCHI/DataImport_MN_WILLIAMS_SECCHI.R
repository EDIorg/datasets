#path to files
setwd("C:/Users/schristel/Dropbox/CSI-LIMNO_DATA/DATA-lake/MN data/USGS_SHAEP_longterm/DataImport_MN_WILLIAMS_SECCHI")
...
487-6#481 obs remain after filtering out those 6 null obs.
data=data[which(is.na(data$Secchi..m.)==FALSE),]
unique(data$Notes) #censor those observations w/ "plus" or "plus; can see on bottom" as "GT"
...
data.Export$LakeID = 29001500  #this is the MN DOWID, the unique lake id assinged by the dnr
data.Export$LakeName = "WILLIAMS"
...
data.Export$Value = data$Secchi..m. #export secchi values, already in preferred units of m
unique(data.Export$Value)#look at exported values
#censor those obs w/ comments as "GT"
unique(data$Notes)
length(data$Notes[which(data$Notes=="plus")])#2
length(data$Notes[which(data$Notes=="plus; can see on bottom")])#10
#12 observations should be censored as GT
data.Export$CensorCode=as.character(data.Export$CensorCode)
data.Export$CensorCode[which(data$Notes=="plus")]="GT"
data.Export$CensorCode[which(data$Notes=="plus; can see on bottom")]="GT"
...
#check to make sure correct number are "GT"
length(data.Export$CensorCode[which(data.Export$CensorCode=="GT")])#12
...
write.table(Final.Export1,file="DataImport_MN_WILLIAMS_SECCHI.csv",row.names=FALSE,sep=",")
save.image("C:/Users/schristel/Dropbox/CSI-LIMNO_DATA/DATA-lake/MN data/USGS_SHAEP_longterm/DataImport_MN_WILLIAMS_SECCHI/DataImport_MN_WILLIAMS_SECCHI.RData")
